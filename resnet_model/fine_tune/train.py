        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                if dl_i == len(dataloaders[phase]) // 2:
                  print('epoch 50% through')
                if dl_i == 3*len(dataloaders[phase]) // 4:
                  print('epoch 75% through')
                if dl_i == len(dataloaders[phase]) - 1:
                  print('epoch 100% through')
...
def feature_extractor(model_ft, dataLoc):
  num_classes = 10
  batch_size = 32
  num_epochs = 5
  ...
  input_size = 224
  device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
  #grab infeature from model
  in_features = model_ft.fc.in_features
  #set grad = false for all
  set_parameter_requires_grad(model_ft, feature_extract)
  # add new classifier layer
  model_ft.fc = nn.Linear(in_features, num_classes, bias=True)

  #load in the data
  dataloaders_dict = load_data(dataLoc, input_size, batch_size)
  # get parameters to learn
...
if __name__ == '__main__':
  ...
  dataDir = Path('data')
  model_ft, hist = feature_extractor(model, dataDir)
